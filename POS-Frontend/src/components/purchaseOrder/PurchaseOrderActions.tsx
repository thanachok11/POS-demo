    ...
    return (
        <div className="po-actions">
            {po.status === "รอดำเนินการ" && (
                <>
                    <button onClick={handleConfirm} disabled={loading}>ยืนยันรับสินค้า</button>
                    <button className="po-cancel-button" onClick={() => openConfirmPopup("ยกเลิกใบสั่งซื้อนี้หรือไม่?", handleCancel)}>ยกเลิก</button>
                </>
            )}
            ...
            {po.status === "ไม่ผ่าน QC - รอส่งคืนสินค้า" && (
                <button className="po-return-button" onClick={() => openConfirmPopup("คืนสินค้า PO นี้หรือไม่?", handleReturn)}>↩️ คืนสินค้า</button>
            )}
        </div>
    );
};
